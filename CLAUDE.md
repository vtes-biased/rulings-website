...
Key env vars (`.env`): `DISCORD_WEBHOOK`, `DISCORD_SERVER_ID`, `ARCHON_URL`, `ARCHON_CLIENT_ID`, `ARCHON_CLIENT_SECRET` (the OAuth client registered on archon; `SITE_URL_BASE` builds the redirect URI archon matches **exactly**). Two archon deployments with separate databases hold separate registrations: **prod is `archon.vekn.net`** (ansible sets `ARCHON_URL`), while the code default is the **beta** `archon.krcg.org` that dev uses. Also read: `SESSION_SECRET_KEY`, `SITE_URL_BASE`, `DATABASE_URL`, `RULINGS_GIT`, `RULINGS_GITHUB_{APP_ID,INSTALLATION_ID,PRIVATE_KEY}`, `KRCG_STATIC_{REPO,INSTALLATION_ID}` (approval→krcg-static rebuild dispatch), `GIT_AUTHOR_{NAME,EMAIL}` (bot identity), and `GIT_SSH_COMMAND` (only if `RULINGS_GIT` is an ssh remote). Vars are read directly via `os.getenv` — notably `TESTING=1` turns `POST /login` into a direct session mint, bypassing archon. Deploy to gravelines lives in `ansible/` (see `ansible/README.md`).
...
- Ruling text embeds: discipline/type **symbols** in brackets `[pot]`, **card names** in braces carrying the card id, `{100006|Abbot}` (editors type `{Abbot}`; `normalize_cards` rewrites it to the id form on save), and **reference ids** in brackets `[LSJ 20040518]`. `utils.py` holds the regexes, the `ANKHA_SYMBOLS` map (text→font glyph), and reference validation (`RULING_AUTHORS` date windows + allowed `RULING_DOMAINS`).

### Users & auth
Login is an OAuth2 authorization-code + PKCE handshake against **archon** (`archon.py`): `GET /login` parks a state and a verifier in the session and sends the user to archon's *frontend* consent page (which forwards its query to `/oauth/authorize` verbatim, so `response_type` and `code_challenge_method` ride in that URL); `GET /login/callback` — declared above the `/{page:path}` catch-all, which would otherwise swallow it — exchanges the code (**JSON body**, archon's divergence from form encoding), reads `/oauth/userinfo` for `{sub, roles, vekn_id, capabilities}` and stores `user_id` in session. No VEKN password ever reaches us; a user with no `vekn_id` is refused. `POST /login` survives as a `TESTING`-only session mint (it takes an `approver` flag, the only seam for testing approval). Users are keyed on the archon uid (`users.archon_uid`); a login with no matching row creates one. `users.vekn` is UNIQUE, mirroring archon's own unique index on `vekn_id` — no collision is reachable once every row comes from archon, so no login path guards against one. Roles are archon's, never ours: `IC` or `Rulemonger` in `roles` sets `users.approver` — the single privilege left, gating approval and seeing other people's proposals — and `api.get_current_user` re-checks it hourly through the stored refresh token. That refresh **rotates with chain revocation**, so exactly one request may spend the stored token: the check is claimed with a conditional `UPDATE … WHERE roles_checked_at IS NOT DISTINCT FROM <what we read>` — no row lock, and no pool connection, held across archon's HTTP. Archon refusing (400) is final: it strips the flag and the token, which ends *every* session of that user (`refresh_token` NULL with `roles_checked_at` set is the dead-chain marker; both NULL just means never checked). Any other failure keeps the roles and defers the retry an hour. `db.init()` is the only migration path — every statement there must be idempotent. CLI (click group `main`, wired to the `rulings-web` script): `rulings-web resetdb`.

## Module map (`src/vtesrulings/`)
- `__init__.py` — FastAPI app, ASGI lifespan (loads cards + clones rulings repo onto `app.state`), page routes, `SessionMiddleware`, StaticFiles mount, Jinja template filters (`symbolreplace`, `cardreplace`, `newlines`), login routes, CLI commands.
...
Frontend lives in `src/front/` (`js/` legacy entrypoints being retired, `island/` Svelte); Vite (`vite.config.ts`) outputs to `src/vtesrulings/static/dist/` (stable entry names `js/index.js`, `js/groups.js`, `js/island.js`, `css/layout.css`). Templates are in `src/vtesrulings/templates/` (`index.html` cards, `groups.html` groups).
